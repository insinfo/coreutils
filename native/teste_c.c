...
FinalizeSections(PMEMORYMODULE module) {
    ...
    for (i = 1; i < module->headers->FileHeader.NumberOfSections; i++, section++) {
        ...
        if (sectionData.alignedAddress == alignedAddress || (uintptr_t) sectionData.address + sectionData.size > (
            ...
            sectionData.size = (((uintptr_t) sectionAddress) + ((uintptr_t) sectionSize)) - (uintptr_t) sectionData.
                               address;
            continue;
        }
...
HMEMORYMODULE MemoryLoadLibrary(const void *data, size_t size) {
    return MemoryLoadLibraryEx(data, size, MemoryDefaultAlloc, MemoryDefaultFree, MemoryDefaultLoadLibrary,
...
HMEMORYMODULE MemoryLoadLibraryEx(const void *data, size_t size,
    ...
    if (result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
        if (result->isDLL) {
            DllEntryProc DllEntry = (DllEntryProc) (LPVOID) (
...
void MemoryFreeLibrary(HMEMORYMODULE mod) {
    ...
    if (module->initialized) {
        ...
        DllEntryProc DllEntry = (DllEntryProc) (LPVOID) (
            module->codeBase + module->headers->OptionalHeader.AddressOfEntryPoint);
...
HMEMORYRSRC MemoryFindResourceEx(HMEMORYMODULE module, LPCTSTR name, LPCTSTR type, WORD language) {
    ...
    typeResources = (PIMAGE_RESOURCE_DIRECTORY) (
        codeBase + directory->VirtualAddress + (foundType->OffsetToData & 0x7fffffff));
    ...
    nameResources = (PIMAGE_RESOURCE_DIRECTORY) (
        codeBase + directory->VirtualAddress + (foundName->OffsetToData & 0x7fffffff));
...
int main(int argc, char **argv) {
    printf("IMAGE_FILE_MACHINE_AMD64 %d\n", sizeof(IMAGE_FILE_MACHINE_AMD64));
    return 0;
}
